...
Object* ObjectLoader::load_objects(string objectFileNames[], int numObjects) {
	...
	glGenBuffers(this->numObjects, this->eba);

	Vector3f colour(0.5, 0.5, 0.5);

	// Load each object into the object list
	for (int i = 0; i < this->numObjects; i++) {

		// Initialise the mesh for each object
		this->objects->mesh = new Mesh(this->vba[i], this->eba[i]);

		if (this->objects->mesh == NULL) {
			log.log_error("Falied to allocated memory for mesh");
		}

		this->objects->mesh->load_mesh(objectFileNames[i], colour);
	}
